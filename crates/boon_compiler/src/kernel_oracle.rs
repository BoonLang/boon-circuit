...
enum PreparedProjectionRetention {
    Runtime,
    Oracle,
}
...
fn prepare_kernel_project_projection(
    ...
    for owner in &mut prepared {
        for synthetic_index in 0..owner.resource_synthetic_paths.len() {
            let synthetic = &owner.resource_synthetic_paths[synthetic_index];
            let anchor = synthetic.anchor.clone();
            let kind = synthetic.kind;
            let row = synthetic.row;
            let ordinal = resource_ordinals.entry((anchor, kind)).or_default();
    ...
    {
        let compact = &prepared[prepared_index].payload().compact;
        eprintln!(
    ...
    // Resolve cross-owner coordinates while the source-shaped payloads are
    // still borrowed in place. The completed payload is moved—not cloned—into
    // the kernel input below.
    for prepared_index in active.iter().copied() {
        let (external_expressions, call_targets) = {
            let owner = &prepared[prepared_index];
            let external_expressions = owner
            ...
            let call_targets = owner
                ...
                .collect::<Vec<_>>();
            (external_expressions, call_targets)
        };
        let compact = &mut prepared[prepared_index].payload_mut().compact;
        compact.external_expressions = external_expressions;
        for (call, dense_target) in call_targets {
            let node = compact
                .nodes
                .get_mut(call)
                .expect("prepared user call node is local");
            let KernelOwnerNodeKind::UserCall { target, .. } = &mut node.kind else {
    ...
    for prepared_index in active.iter().copied() {
        ...
        let (statement_children, lexical_targets, resource_targets) = {
            let owner = &prepared[prepared_index];
            let statement_children = owner
                ...
                .map(|target| {
                    ...
                    (
                        target.statement,
                        target.child,
                        dense_owner[prepared_target]
                            .expect("active statement target has a dense owner"),
                    )
                })
                .collect::<Vec<_>>();
            let lexical_targets = owner
                .lexical_owner_targets
                .iter()
                .map(|target| {
                    let prepared_target = prepared_by_owner[&target.owner];
                    let dense_target = dense_owner[prepared_target]
                        .expect("active lexical target has a dense owner");
                    let declaration = target.declaration.as_ref().map(|(arm, ordinal)| {
                    ...
                    (target.binding, dense_target, declaration)
                })
                .collect::<Vec<_>>();
            let resource_targets = owner
                .resource_owner_targets
                .iter()
                .map(|target| {
                    let prepared_target = prepared_by_owner[&target.owner];
                    let dense_target = dense_owner[prepared_target]
                        .expect("active resource target has a dense owner");
                    let parent_authority =
                        matches!(target.field, PreparedResourceOwnerField::ListStatement(_))
                            && prepared_inline_list_authority_owner(
                                &prepared,
                                &active,
                                dense_target,
                            ) == Some(dense_current);
                    (target.field, dense_target, parent_authority)
                })
                .collect::<Vec<_>>();
            (statement_children, lexical_targets, resource_targets)
        };

        let facts = &mut prepared[prepared_index].payload_mut().definition_facts;
        ...
        for (statement, child, dense_target) in statement_children {
            let child = facts.statements[statement]
                .children
                .get_mut(child)
        ...
        for (binding, dense_target, external_declaration) in lexical_targets {
            let binding = facts
                .lexical_bindings
                .get_mut(binding)
                .expect("prepared lexical owner target is in range");
            match (&mut binding.target, external_declaration) {
                (
        ...
        for (field, dense_target, parent_authority) in resource_targets {
            match field {
    ...
    let project_is_empty = active.is_empty();
    let definition_keys = active
        .iter()
        .map(|prepared_index| prepared[*prepared_index].owner.clone())
        .collect::<Vec<_>>()
        .into_boxed_slice();
    let mut project_owners = Vec::with_capacity(active.len());
    let mut definition_facts = Vec::with_capacity(active.len());
    for prepared_index in active.iter().copied() {
        let payload = prepared[prepared_index]
            .payload
            .take()
            .expect("active prepared owner has one compiler payload");
        if retention == PreparedProjectionRetention::Oracle {
            prepared[prepared_index].payload = Some(payload.clone());
        }
        project_owners.push(payload.compact);
        definition_facts.push(payload.definition_facts);
    }
    if retention == PreparedProjectionRetention::Runtime {
        for owner in &mut prepared {
            owner.payload = None;
        }
    }
    let project_input = KernelProjectProgramInput {
        owners: project_owners.into_boxed_slice(),
    };
    let definition_facts = definition_facts.into_boxed_slice();
    PreparedKernelProjectProjection {
        owner_order,
        input_owners,
        prepared,
...
fn profile_kernel_owner_oracle_with_source_payloads_for_role(
    ...
    let PreparedKernelProjectProjection {
        ...
        direct_projection_elapsed,
        dependency_pruning_us,
    } = prepare_kernel_project_projection(
        project,
        source_payloads,
        role,
        PreparedProjectionRetention::Oracle,
    );
    ...
    let (supported, work, dependency_edges, reverse_consumer_edges, currentness) = artifact
        ...
        |snapshot| {
            ...
            let dependency_edges = snapshot.dependencies().dependency_count();
            let reverse_consumer_edges = snapshot.dependencies().reverse_consumer_count();
            let currentness = active
                .iter()
                .zip(snapshot.currentness())
            ...
            let supported = active
                .iter()
                .zip(snapshot.definition_refs())
                .enumerate()
                .map(|(dense_index, (prepared_index, definition))| {
                    ...
                    let stable_provider = |value: KernelValueReference| match value {
                        KernelValueReference::Local(expression) => {
                            KernelOwnerOracleValueReference::Expression(
                                owner.expression(expression).clone(),
                            )
                        }
                        KernelValueReference::External(external) => {
                            ...
                            match external.target {
                                KernelExternalTarget::Expression(expression) => {
                                    KernelOwnerOracleValueReference::Expression(
                                        target.expression(expression).clone(),
                                    )
                                }
                        ...
                        .filter_map(|(ordinal, expression)| {
                            let expression_id = KernelExpressionId(
                                u32::try_from(ordinal)
                                    .expect("kernel expression count exceeds u32"),
                            );
                            let boon_compiler_kernel::KernelExpressionKindRef::Collection {
                                kind,
                                capacity,
                            } = definition
                                .expression_kind(expression_id)
                                .expect("kernel node owns an opcode view")
                            else {
                                return None;
                            };
                            Some(KernelOwnerOracleCollection {
                                expression: owner.expression_at(ordinal).clone(),
                                kind,
                                capacity,
                                inputs: expression
                                    .inputs
                                    ...
                                    .map(|input| KernelOwnerOracleExpressionInput {
                                        ...
                                        provider: stable_provider(
                                            definition
                                                ...
                                                .expect("validated collection input resolves"),
                                        ),
                                    })
                                    .collect::<Vec<_>>()
                                    .into_boxed_slice(),
                                flow_type: code
                                    .materialize_expression(ordinal)
                    ...
                    let sources = definition
                        ...
                        .filter_map(|(ordinal, _)| {
                            ...
                            let boon_compiler_kernel::KernelExpressionKindRef::Source = definition
                                .expression_kind(expression_id)
                                .expect("kernel node owns an opcode view")
                            else {
                                return None;
                            ...
                            Some(KernelOwnerOracleSource {
                                expression: owner.expression_at(ordinal).clone(),
                                payload_type: code
                                    .materialize_expression_kind_type(ordinal)
                    ...
                    assert_eq!(
                        ...
                        "kernel statement facts retain one stable row each"
                    );
                    let statements = facts
                        .statements
                        .iter()
                        .map(|statement| KernelOwnerOracleStatement {
                            statement: owner.statement(statement.id).clone(),
                            kind: statement.kind.clone(),
                            value: statement.value.map(|value| {
                                stable_provider(
                                    definition
                                        .resolve_value(value, statement.id.0 as usize)
                                        .expect("validated statement value resolves"),
                                )
                            }),
                            children: statement
                                .children
                    ...
                    let declarations = facts
                        ...
                        .map(|declaration| KernelOwnerOracleDeclaration {
                            ...
                            value: declaration.value.map(|value| {
                                stable_provider(
                                    definition
                                        .resolve_value(value, declaration.id.0 as usize)
                                        .expect("validated declaration value resolves"),
                                )
                            }),
                        })
                        .collect::<Vec<_>>()
                        .into_boxed_slice();
                    let lexical_bindings = facts
                        .lexical_bindings
                        .iter()
                        .map(|binding| {
                            let target = match definition
                                .resolve_lexical_target(binding)
                                .expect("validated lexical target resolves")
                            {
                                boon_compiler_kernel::KernelLexicalBindingTargetRef::Declaration(
                                ...
                                boon_compiler_kernel::KernelLexicalBindingTargetRef::ContextFormal {
                                    ordinal,
                                } => {
                                ...
                                boon_compiler_kernel::KernelLexicalBindingTargetRef::Value {
                                    provider,
                                } => {
                                    KernelOwnerOracleLexicalTarget::Value(stable_provider(provider))
                                }
                                boon_compiler_kernel::KernelLexicalBindingTargetRef::RuntimeContext => {
                                    KernelOwnerOracleLexicalTarget::RuntimeContext
                                }
                            };
                            KernelOwnerOracleLexicalBinding {
                                expression: owner.expression(binding.expression).clone(),
                                target,
                    ...
                    let stable_path =
                        |anchor: KernelDeclarationReference, projection: &[Box<str>]| {
                        match anchor {
                            KernelDeclarationReference::Local(declaration) => {
                    ...
                    let stable_statement = |reference: KernelStatementReference| match reference {
                        KernelStatementReference::Local(statement) => {
                            owner.statement(statement).clone()
                        }
                    ...
                    let source_resources = facts
                        ...
                        .map(|(ordinal, source)| KernelOwnerOracleSourceResource {
                            ...
                            expression: owner.expression(source.expression).clone(),
                            path: stable_path(source.declaration, &source.projection),
                    ...
                    let lists = facts
                        ...
                        .map(|(ordinal, list)| KernelOwnerOracleList {
                            ...
                            producer: owner.expression(list.producer).clone(),
                            path: stable_path(list.declaration, &list.projection),
                            item_type: code
                    ...
                    let diagnostics = interface_diagnostics
                            ...
                            let site = match &diagnostic.site {
                                ...
                                KernelDiagnosticSite::CallArgument { call, source } => {
                                    KernelOwnerOracleDiagnosticSite::CallArgument {
                                        call: owner.expression(*call).clone(),
                                        source: *source,
                                    }
                                }
                        ...
                        .collect::<Vec<_>>()
                        .into_boxed_slice();
                    let mut call_syntax = facts
                        .call_syntax
                        .iter()
                        .map(|syntax| (syntax.expression, syntax))
                        .collect::<BTreeMap<_, _>>();
                    let calls = (0..definition.call_count())
                        .map(|ordinal| {
                            let call = definition
                                .call(ordinal)
                                .expect("packed call coordinate remains resolvable");
                            ...
                            let syntax = call_syntax.remove(&expression).unwrap_or_else(|| {
                                panic!(
                            ...
                            let target = match call.target() {
                                boon_compiler_kernel::KernelCallTargetRef::User {
                                    target,
                                    inherited_formal,
                                } => {
                                    let target = active
                                ...
                                boon_compiler_kernel::KernelCallTargetRef::FixedAbi => {
                                    KernelOwnerOracleCallTarget::FixedAbi
                                }
                                boon_compiler_kernel::KernelCallTargetRef::HostEffect {
                                    operation,
                                } => {
                                    KernelOwnerOracleCallTarget::HostEffect(operation.into())
                                }
                                boon_compiler_kernel::KernelCallTargetRef::FieldProjection {
                                    field,
                                } => {
                                    KernelOwnerOracleCallTarget::FieldProjection(field.into())
                                }
                            };
                            KernelOwnerOracleCall {
                                expression: owner.expression(expression).clone(),
                                function: syntax.function.clone(),
                                pipe_input: syntax.pipe_input.map(|value| {
                                    stable_provider(
                                        definition
                                            .resolve_value(value, expression.0 as usize)
                                            .expect("validated call pipe input resolves"),
                                    )
                                }),
                                arguments: syntax
                                    .arguments
                                    ...
                                    .map(|argument| KernelOwnerOracleCallSyntaxArgument {
                                        ...
                                        provider: stable_provider(
                                            definition
                                                ...
                                                .expect("validated call argument resolves"),
                                        ),
                                    })
                                    .collect::<Vec<_>>()
                                    .into_boxed_slice(),
                                pass: syntax.pass.map(|pass| KernelOwnerOracleCallPass {
                                    provider: stable_provider(
                                        definition
                                            .resolve_value(pass.value, expression.0 as usize)
                                            .expect("validated call PASS value resolves"),
                                    ),
                                ...
                                inputs: call
                                    ...
                                    .map(|input| {
                                        let role = match call
                                            .input_role(input)
                                            .expect("validated call input role resolves")
                                        {
                                            boon_compiler_kernel::KernelCallInputRoleRef::Formal {
                                                ordinal,
                                            } => KernelCallInputRole::Formal { ordinal },
                                            boon_compiler_kernel::KernelCallInputRoleRef::Abi {
                                                name,
                                        ...
                                        KernelOwnerOracleCallInput {
                                            ...
                                            provider: stable_provider(
                                                call.input_value(input)
                                                    .expect("validated call input resolves"),
                                            ),
                                        }
                                    })
                                    .collect::<Vec<_>>()
                    ...
                    let effects = definition
                        .materialize_host_effects()
                        .expect("validated host effects remain materializable")
                        .into_iter()
                        .map(|effect| {
                            (owner.expression(effect.expression).clone(), effect)
                        })
                        .collect::<Vec<_>>()
                        .into_boxed_slice();
                    let expressions = owner
                        ...
                        .map(|(ordinal, source)| {
                            (
                                source.clone(),
                                code.materialize_expression(ordinal)
...
pub(crate) fn compiler_diagnostics_from_kernel(
    ...
    let dense_owner_count = definition_keys.len();
    let owner_count = dense_owner_count.saturating_add(container_owners.len());
    ...
    drop(source_payloads);
    drop(owner_order);
    drop(container_owners);
    drop(unsupported);
    ...
    let owner_work = crate::CompilerOwnerWork {
        ...
        interface_imports: 0,
        interface_plan_direct_owners: u64::try_from(dense_owner_count).unwrap_or(u64::MAX),
        interface_plan_required_owners: u64::try_from(dense_owner_count).unwrap_or(u64::MAX),
...
fn present_kernel_interface_diagnostic(
    ...
    let site = match diagnostic.site() {
        ...
        KernelDiagnosticSite::CallInput {
            ...
            let target = kernel_project_definition_key(input, *target)?;
            KernelOwnerOracleDiagnosticSite::CallInput {
                call: expression(*call)?,
                target: target.clone(),
                formal_ordinal: *formal_ordinal,
        ...
        let (source_owner, source_expression) = match value.value() {
            ...
            KernelValueReference::External(external) => {
                ...
                let expression = match external.target {
                    KernelExternalTarget::Expression(expression) => {
                        kernel_project_expression_key(input, external.owner, expression)?
                    }
...
fn kernel_project_definition_key(
    input: &KernelProjectInput,
    owner: KernelOwnerId,
) -> Result<&StableCheckOwnerKey, String> {
    input
        .links()
        .definition_key(owner)
        .ok_or_else(|| format!("kernel project references missing dense owner {}", owner.0))
}

fn kernel_project_expression_key(
    input: &KernelProjectInput,
    owner: KernelOwnerId,
    ...
    match relocation {
        KernelExpressionRelocation::Authored(expression) => Ok(expression),
        KernelExpressionRelocation::SyntheticDefinitionResult => Err(format!(
            "kernel owner {owner_key:?} uses its synthetic result as a source diagnostic site"
        )),
    }
}

fn kernel_project_render_slot_name<'a>(
    ...
    let local_statement = *view.statement_ids().get(dense).ok_or_else(|| {
        format!("kernel render owner {owner_key:?} has missing local statement ID {dense}")
    })?;
    let stable_statement = view
        .stable_statement_key_local(local_statement)
        .ok_or_else(|| {
    ...
    let slot = match &syntax.kind {
        ...
        _ => {
            return Err(format!(
                "kernel render owner {owner_key:?} statement {dense} has a render value-use without a named field",
            ));
        }
    };
    if actual_value != expected_value {
        return Err(format!(
            "kernel render owner {owner_key:?} diagnostic value {ordinal} is detached from its slot"
        ));
    }
    Ok(slot)
}
...
fn kernel_render_slot_name<'a>(
    ...
    let inherited = match owner {
        ...
        StableCheckOwnerKey::Item(owner) => {
            ...
            segments
                .iter()
                // The final route segment is the statement being classified,
                // not an enclosing render context. Counting it made an
                // ordinary top-level field named `items` classify itself as
                // a render slot.
...
struct PreparedOwner {
    owner: StableCheckOwnerKey,
    /// The source-shaped compiler payload exists only while dense links are
    /// resolved. Production moves it into `KernelProjectInput`; the explicit
    /// differential oracle may retain one post-relocation copy for exact
    /// kernel-input-to-artifact checks.
    payload: Option<PreparedOwnerPayload>,
...
#[derive(Clone, Debug)]
struct PreparedOwnerPayload {
    compact: KernelOwnerProgramInput,
    definition_facts: KernelDefinitionFactsInput,
}

impl PreparedOwner {
    fn payload(&self) -> &PreparedOwnerPayload {
        self.payload
            .as_ref()
            .expect("prepared owner compiler payload has already been consumed")
    }

    fn payload_mut(&mut self) -> &mut PreparedOwnerPayload {
        self.payload
            .as_mut()
            .expect("prepared owner compiler payload has already been consumed")
    }

    fn expression(&self, expression: KernelExpressionId) -> &StableExpressionKey {
        self.expression_at(expression.0 as usize)
    }

    fn expression_at(&self, ordinal: usize) -> &StableExpressionKey {
        match self
            .payload()
            .definition_facts
            .relocations
            .expressions
            .get(ordinal)
            .expect("prepared owner expression relocation is in range")
        {
            KernelExpressionRelocation::Authored(expression) => expression,
            KernelExpressionRelocation::SyntheticDefinitionResult => {
                panic!("synthetic definition results have no authored source key")
            }
        }
    }

    fn expressions(&self) -> impl Iterator<Item = &StableExpressionKey> {
        self.payload()
            .definition_facts
            .relocations
            .expressions
            ...
            .filter_map(|relocation| match relocation {
                KernelExpressionRelocation::Authored(expression) => Some(expression),
                KernelExpressionRelocation::SyntheticDefinitionResult => None,
            })
    }

    fn statement(&self, statement: KernelStatementId) -> &StableStatementKey {
        self.payload()
            .definition_facts
            .relocations
            .statements
            .get(statement.0 as usize)
            .expect("prepared owner statement relocation is in range")
    }

    fn statements(&self) -> &[StableStatementKey] {
        &self.payload().definition_facts.relocations.statements
    }
}
            ...
            (
                ...
                AuthoritativeCallSurface {
                    ...
                    parameters: callable
                        ...
                        .collect::<Vec<_>>()
                        .into_boxed_slice(),
                    fixed_result: (callable.kind == KernelCallableKind::Builtin
                        && callable.parameters.is_empty()
                        && callable.contexts.is_empty()
                        && callable.result_specialization == KernelAbiResultSpecialization::Fixed
                        && type_is_recursively_closed(&callable.result.ty)
                        && callable.effect == Default::default())
                    .then(|| callable.result.ty.clone()),
                },
            )
        })
        .collect()
...
fn compact_owner_view(
        ...
        AstStatementKind::Function { parameters, .. } => {
            ...
            for parameter in parameters {
                if parameter.ordinal >= parameters.len()
                    || by_name
                        .insert(parameter.name.as_str(), parameter.ordinal)
                        .is_some()
                {
            ...
            if let Some(context_ordinal) = owner_context_ordinal {
                ...
                if by_name.insert("PASSED", formal_count).is_some() {
    ...
    let result_expression = (retention == PreparedProjectionRetention::Oracle)
        .then(|| {
            (root_statement.expr.is_some()
    ...
    // These checks define whether an owner is structurally supported, so they
    // run for Runtime and Oracle alike. Only the differential report rows
    // below are retention-dependent.
    ...
    let mut direct_child_owner_by_name = BTreeMap::<&str, StableCheckOwnerKey>::new();
    if let StableCheckOwnerKey::Item(parent_owner) = &owner {
        for boundary in view.child_owners() {
            ...
            let name = match &statement.kind {
                AstStatementKind::Field { name } => Some(name.as_str()),
            ...
            if let Some(name) = name
                && let Some(previous) = direct_child_owner_by_name.insert(name, child_owner.clone())
                && previous != child_owner
            {
                return Err(format!(
    ...
    let public_child_owner_fields = if retention == PreparedProjectionRetention::Oracle {
                    ...
                    let child_owner =
                        direct_child_owner_by_name
                            ...
                            .or_else(|| match value {
                                ...
                                PreparedInputReference::Syntax(value) => {
                                    child_owner_by_value.get(value)
                                }
                            })?;
        ...
        public_child_owner_fields.into_boxed_slice()
    } else {
        Box::new([])
    };
    ...
    let structured_delimiter_nodes = if retention == PreparedProjectionRetention::Oracle {
        structured_records
            .keys()
            .filter_map(|syntax| {
            ...
            structured_records.get(&expression.id)
        {
            ...
            match &expression.kind {
                ...
                AstExprKind::Identifier(name) => {
                    ...
                    } else if let Some(formal) = formal_by_name.get(name.as_str()).copied() {
                        (
                ...
                AstExprKind::Path(_) | AstExprKind::Drain { .. } => {
                    ...
                    if let Some(binding) = lexical_binding_reads.get(&expression.id) {
                        ...
                        (kind, edges, None, None)
                    } else if let Some(formal) = formal_by_name.get(root.as_str()).copied() {
                        (
                            if Some(formal) == owner_context_ordinal {
                                KernelOwnerNodeKind::ContextRead {
                                    formal: checked_u32(formal, "context formal ordinal")?,
                ...
                AstExprKind::Call {
                    ...
                    && authoritative_call_shapes
                        .get(function)
                        .is_some_and(|surface| surface.fixed_result.is_some()) =>
                {
                    let result = authoritative_call_shapes[function]
                        .fixed_result
                        .clone()
                        .expect("fixed-result ABI call guard resolves");
                    (
    ...
    let (
        ...
        let structured_delimiter_dependents =
            local_dependency_cone(&nodes, structured_delimiter_nodes)
                .into_iter()
                .map(|index| expressions[index].clone())
                .collect::<Vec<_>>()
                .into_boxed_slice();
        ...
        let detached_generic_reads = nodes
            ...
            .collect::<Vec<_>>()
            .into_boxed_slice();
        // `NoElement` is an ordinary tag in Boon. This legacy compatibility
        // cone is built only for the differential oracle; production never
        // classifies the spelling specially.
        ...
        let empty = || Vec::<StableExpressionKey>::new().into_boxed_slice();
    ...
    let generic_formal_reads = if retention == PreparedProjectionRetention::Oracle {
        nodes
            .iter()
            .enumerate()
            .filter_map(|(index, node)| {
    ...
    definition_facts.call_syntax = raw_expressions
        ...
        .collect::<Result<Vec<_>, _>>()?
        .into_boxed_slice();
    let render_slots = definition_facts
        .statements
        .iter()
        .filter_map(|fact| {
            (fact.value_use == KernelStatementValueUse::RenderSlot)
        ...
        .collect::<Vec<KernelExpressionId>>();
    definition_facts.diagnostic_values = render_slots.into_boxed_slice();
    definition_facts.relocations.statements = statements;
    ...
    Ok(PreparedOwner {
        owner,
        payload: Some(PreparedOwnerPayload {
            definition_facts,
            compact: KernelOwnerProgramInput {
...
fn kernel_resource_projection(
    ...
    fn visit(
        ...
        let result = node
            .inputs
            .iter()
            .filter(|input| match &node.kind {
                KernelOwnerNodeKind::Record { .. } => matches!(
                    input.role,
                    KernelOwnerEdgeRole::RecordField { spread: false, .. }
                ),
                KernelOwnerNodeKind::Block => input.role == KernelOwnerEdgeRole::BlockResult,
                KernelOwnerNodeKind::Collection { .. } => {
                    input.role == KernelOwnerEdgeRole::CollectionItem
                }
                KernelOwnerNodeKind::MapEntry => input.role == KernelOwnerEdgeRole::MapValue,
                KernelOwnerNodeKind::PureBuiltin {
                    kind: KernelPureBuiltinKind::BoolToggle,
                } => matches!(
                    &input.role,
                    KernelOwnerEdgeRole::AbiArgument { name }
                        if matches!(name.as_ref(), "$pipe" | "value")
                ),
                KernelOwnerNodeKind::Latest => input.role == KernelOwnerEdgeRole::LatestBranch,
                KernelOwnerNodeKind::When => input.role == KernelOwnerEdgeRole::WhenArm,
                KernelOwnerNodeKind::Then => input.role == KernelOwnerEdgeRole::ThenOutput,
...
/// Whether a state expression is the declaration's published result after
/// crossing only result-transparent structural wrappers.
///
/// Inputs consumed by calls, THEN, WHEN, records, or collections are not
/// declaration results merely because they are reachable in the dependency
/// graph. Such nested cells receive a synthetic `state_N` path.
fn kernel_state_is_declaration_result(
    nodes: &[KernelOwnerNode],
    root: KernelExpressionId,
    target: KernelExpressionId,
) -> bool {
    ...
    while visited.insert(current) {
        ...
        let Some(node) = nodes.get(current.0 as usize) else {
            return false;
        };
        let next = match &node.kind {
            KernelOwnerNodeKind::Draining => node
                .inputs
                .iter()
                .find(|input| input.role == KernelOwnerEdgeRole::DrainingInput),
            KernelOwnerNodeKind::Block => node
                .inputs
                .iter()
                .find(|input| input.role == KernelOwnerEdgeRole::BlockResult),
            KernelOwnerNodeKind::MatchArm { .. } => node
                .inputs
                .iter()
...
/// Find the first authored record field on the definition result path to a
/// nested resource.
///
/// A resource expression can be nested below several anonymous record values.
/// Checked lexical reads name the shallowest authored field that contains the
/// resource and retain the remaining structural route as the resource path.
/// The innermost field is only the payload occurrence; using it as the public
/// authority loses whole-container reads. Explicit SOURCE/HOLD/LIST statements
/// are selected before this helper and remain stronger authorities.
fn shallowest_resource_declaration<'a>(
    nodes: &[KernelOwnerNode],
    result: KernelExpressionId,
    resource: KernelExpressionId,
    declarations: &'a [KernelDeclarationInput],
) -> Result<Option<&'a KernelDeclarationInput>, String> {
    let Some(full_path) = kernel_resource_projection(nodes, result, resource) else {
        return Ok(None);
    };
    let mut selected = None::<(&KernelDeclarationInput, (usize, bool))>;
    for declaration in declarations {
        ...
        let Some(prefix) = kernel_resource_projection(nodes, result, value) else {
            continue;
        };
        let Some(suffix) = kernel_resource_projection(nodes, value, resource) else {
            continue;
        };
        if prefix.iter().chain(suffix.iter()).ne(full_path.iter()) {
            continue;
        }
        let score = (suffix.len(), value == resource);
        match selected {
            None => selected = Some((declaration, score)),
            Some((_, selected_score)) if score > selected_score => {
                selected = Some((declaration, score));
            }
            Some((previous, selected_score))
                if score == selected_score && previous.id != declaration.id =>
            {
                return Err(format!(
                    "resource expression {} has ambiguous shallow structural declarations {} `{}` {:?} value {:?} and {} `{}` {:?} value {:?}",
                    resource.0,
                    previous.id.0,
                    previous.name,
                    previous.kind,
    ...
    Ok(selected.map(|(declaration, _)| declaration))
}
...
fn prepared_inline_list_authority_owner(
    prepared: &[PreparedOwner],
    active: &[usize],
    owner: KernelOwnerId,
) -> Option<KernelOwnerId> {
    let input = &prepared
        .get(*active.get(owner.0 as usize)?)?
        .payload()
        .compact;
    ...
    while visited.insert(current) {
        ...
        if index >= input.nodes.len() {
            ...
            return match external.target {
                ...
                KernelExternalTarget::Expression(expression) => {
                    let target = &prepared
                        .get(*active.get(external.owner.0 as usize)?)?
                        .payload()
                        .compact;
...
> {
    ...
    let canonical_projection = |target: &PreparedLexicalTarget,
     ...
     -> (Vec<Box<str>>, bool) {
        ...
        let root = declaration
            .value
            .or_else(|| (declaration.kind == KernelDeclarationKind::Function).then_some(result));
    ...
    for expression in raw_expressions {
        ...
        match &nodes[dense.0 as usize].kind {
            KernelOwnerNodeKind::Source(_) => {
                // A structured SOURCE statement owns the full authored field;
                // its payload expression can sit below one or more record
                // fields. The statement declaration remains the lexical
                // authority and the nested route belongs in `projection`.
                // Choosing the innermost record-field declaration here leaves
                // whole-container reads without a semantic field/source
                // binding and disagrees with the checked language contract.
                let exact_statement_target = declarations_by_value
                    .get(&dense)
                    .and_then(|declarations| {
                        declarations.iter().find(|declaration| {
                            declaration.kind == KernelDeclarationKind::Source
                                && matches!(
                                    declaration.origin,
                                    KernelDeclarationOrigin::Statement { .. }
                                )
                ...
                let structural_target =
                    shallowest_resource_declaration(nodes, result, dense, &facts.declarations)?
                        .map(|declaration| {
                            PreparedLexicalTarget::Declaration(declaration.origin.clone())
                        });
                let Some(target) = exact_statement_target
                    .or(structural_target)
                    .or_else(|| canonical_target(dense, false, false))
                else {
                    continue;
                };
            ...
            | KernelOwnerNodeKind::PureBuiltin {
                ...
                let structural_target =
                    (!matches!(nodes[dense.0 as usize].kind, KernelOwnerNodeKind::Hold))
                        .then(|| {
                            shallowest_resource_declaration(
                                nodes,
                                result,
                                dense,
                                &facts.declarations,
                ...
                let Some(target) =
                    structural_target.or_else(|| canonical_target(dense, true, false))
                else {
                    continue;
                };
                let Some(declaration) = local_reference(&target) else {
                ...
                let (kind, initial, binding_target) = match &nodes[dense.0 as usize].kind {
                    ...
                    KernelOwnerNodeKind::PureBuiltin {
                        ...
                        let Some(initial) = nodes[dense.0 as usize]
                            ...
                            return Err(format!(
                                "stateful ABI expression {} has no initializer input",
                                expression.id,
                            ));
                        };
                        (CheckedStateKind::StatefulCall, initial, target.clone())
                    }
                    _ => unreachable!("state match accepts only state-owning expressions"),
                };
                let Some(binding_declaration) = local_reference(&binding_target) else {
                    continue;
                };
                let (projection, _) = canonical_projection(&target, dense);
                let (declaration_state_authority, function_declaration) = match &target {
                    PreparedLexicalTarget::Declaration(origin) => declaration_by_origin
                        .get(origin)
                        .and_then(|declaration| facts.declarations.get(declaration.0 as usize))
                        .map_or((false, false), |declaration| {
                            let root = declaration.value.or_else(|| {
                                (declaration.kind == KernelDeclarationKind::Function)
                                    .then_some(result)
                            });
                            (
                                root.is_some_and(|root| {
                                    kernel_state_is_declaration_result(nodes, root, dense)
                                }),
                                declaration.kind == KernelDeclarationKind::Function,
                            )
                        }),
...
fn pure_builtin_kind(function: &str) -> Option<KernelPureBuiltinKind> {
    Some(match function {
        ...
        "Bool/not" | "Bool/and" | "Bool/or" => KernelPureBuiltinKind::Boolean,
        "Bool/toggle" => KernelPureBuiltinKind::BoolToggle,
...
mod tests {
    ...
    fn owner_mismatch(
        ...
        let contextual_delimiters =
            project
                .owner_view(&owner.owner)
                .map_or_else(BTreeSet::new, |view| {
                    view.expressions()
                        .zip(view.stable_expression_keys())
                        .filter_map(|(expression, stable)| {
                            matches!(expression.kind, AstExprKind::Delimiter).then_some(stable)
                        })
                        .collect::<BTreeSet<_>>()
                });
        ...
        let legacy_contextual_delimiter_result = owner
            .result_expression
            .as_ref()
            .is_some_and(|result| contextual_delimiters.contains(result));
        ...
        let compared = owner
            ...
            .map(|(stable_key, flow)| {
                ...
                (
                    ...
                    contextual_delimiters.contains(stable_key),
                )
            })
            .collect::<Vec<_>>();
        ...
        let mut legacy_contextual_delimiter_expressions = Vec::with_capacity(compared.len());
        ...
        for (
            ...
            generic_selector,
            legacy_no_element,
            legacy_source_container,
            legacy_contextual_delimiter,
        ) in compared
        {
            ...
            legacy_contextual_delimiter_expressions.push(legacy_contextual_delimiter);
        }
        ...
        let result_matches = result_exact
            ...
            || (legacy_contextual_delimiter_result
                && legacy_contextual_delimiter_matches(&kernel_result, &current_result));
            ...
            .enumerate()
        {
            let matches = flow_matches_current_or_legacy_render_projection(kernel, current)
                ...
                || (legacy_contextual_delimiter_expressions[index]
                    && legacy_contextual_delimiter_matches(kernel, current));
            if !matches {
                let syntax_kind = project.owner_view(&owner.owner).and_then(|view| {
                    view.expressions()
                        .zip(view.stable_expression_keys())
                        .find_map(|(expression, stable)| {
                            (stable == *compared_keys[index])
                                .then(|| format!("{:?}", expression.kind))
                        })
                });
                return Some(format!(
                    "{context} expression {index} ({:?}, syntax={syntax_kind:?}) mismatch: {}",
                    compared_keys[index],
    ...
    fn direct_callable_inventory_mismatches(
        report: &KernelOwnerOracleReport,
        checked: &CheckedProgramFields,
        project: &ProjectSyntaxSnapshot,
    ) -> Vec<String> {
        let mut mismatches = Vec::new();
        let current_user_count = checked
            .callables
            .iter()
        ...
        let direct_statement_stable_by_id = report
            .checked_statement_keys
            .iter()
            .cloned()
            .zip(
            ...
            .map(|(stable, id)| (id, stable))
            .collect::<BTreeMap<_, _>>();
        let direct_expression_stable_by_id = report
            .checked_expression_keys
            .iter()
            .zip(&report.checked_expressions)
            .filter_map(|(stable, expression)| Some((expression.id, stable.as_ref()?.clone())))
            .collect::<BTreeMap<_, _>>();
        let mut current_statement_by_stable = BTreeMap::new();
        let mut current_expression_stable_by_id = BTreeMap::new();
        for owner in project.stable_check_owner_keys() {
            ...
            for statement in view.statement_ids() {
                ...
                current_statement_by_stable.entry(stable).or_insert(current);
            }
            for (expression, stable) in view.expressions().zip(view.stable_expression_keys()) {
                ...
                current_expression_stable_by_id
                    .entry(current.id)
                    .or_insert(stable);
            }
        }

        let declaration_signature = |declaration: &CheckedDeclaration| {
            (declaration.kind, declaration.name.clone(), declaration.span)
        };
        let output_ordinal =
            |scope: boon_checked::CheckedEvaluationScope,
             parameters: &[boon_checked::CheckedParameter]| {
                match scope {
                    boon_checked::CheckedEvaluationScope::Parent => Ok(None),
                    boon_checked::CheckedEvaluationScope::Output { formal } => parameters
                        .iter()
                        .find(|parameter| parameter.decl_id == formal)
                        .map(|parameter| Some(parameter.ordinal))
                        .ok_or(formal),
                }
            };
            ...
            .filter(|callable| callable.kind == boon_checked::CheckedCallableKind::User)
        {
            ...
            let Some(stable_body) = direct_statement_stable_by_id.get(&direct_body) else {
                mismatches.push(format!(
                    "kernel direct user callable {:?} body {direct_body:?} has no stable identity",
                    direct.name,
                ));
                continue;
            };
            let Some(current_statement) = current_statement_by_stable.get(stable_body).copied()
            else {
                mismatches.push(format!(
                    "kernel direct user callable {:?} body {stable_body:?} has no checked row",
                    direct.name,
                ));
                continue;
            };
            let boon_checked::CheckedStatementKind::Function {
                declaration: current_declaration_id,
            } = current_statement.kind
            else {
                mismatches.push(format!(
                    "kernel direct callable body {stable_body:?} maps to non-function checked statement {:?}",
                    current_statement.kind,
                ));
                continue;
            };
            let Some(current) = checked
                .callables
                .iter()
                .find(|callable| callable.decl_id == current_declaration_id)
            else {
                mismatches.push(format!(
                    "checked function {stable_body:?} has no callable signature",
                ));
                continue;
            };
            if current.kind != boon_checked::CheckedCallableKind::User {
                mismatches.push(format!(
                    "kernel user callable {stable_body:?} maps to checked {:?}",
                    current.kind,
                ));
                continue;
            }
            ...
            let direct_declaration = report
                .checked_declarations
                .iter()
                .find(|declaration| declaration.id == direct.decl_id);
            let current_declaration = checked
                .declarations
                .iter()
                .find(|declaration| declaration.id == current.decl_id);
            if direct_declaration.map(&declaration_signature)
                != current_declaration.map(&declaration_signature)
            {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} declaration differs: direct={direct_declaration:?} checked={current_declaration:?}",
                ));
            }
            let direct_scope = report
                .checked_scopes
                .get(direct.scope_id.0 as usize)
                .map(|scope| (scope.kind, scope.span));
            let current_scope = checked
                .scopes
                .get(current.scope_id.0 as usize)
                .map(|scope| (scope.kind, scope.span));
            if direct_scope != current_scope {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} scope differs: direct={direct_scope:?} checked={current_scope:?}",
                ));
            }

            let direct_context = direct.context_formal.and_then(|id| {
                report
                    .checked_context_formals
                    .iter()
                    .find(|formal| formal.id == id)
            });
            let current_context = current
                .context_formal
                .and_then(|id| checked.context_formal(id));
            let mut direct_surface = direct
                .parameters
                .iter()
                .map(|parameter| parameter.flow_type.clone())
                .collect::<Vec<_>>();
            if let Some(context) = direct_context {
                direct_surface.push(context.scheme.flow_type.clone());
            }
            let direct_surface = alpha_normalize_callable_surface(&direct_surface, &direct.result);
            let Some(owner) = report.supported.iter().find(|owner| {
                owner
                    .statements
                    .iter()
                    .any(|statement| &statement.statement == stable_body)
            }) else {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} has no definition artifact",
                ));
                continue;
            };
            let definition_surface =
                alpha_normalize_callable_surface(&owner.formals, &owner.result);
            if direct_surface != definition_surface {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} alpha surface differs from its definition: direct={direct_surface:?} definition={definition_surface:?}",
                ));
            }

            let stable_result = direct
                .result_expression
                .and_then(|expression| direct_expression_stable_by_id.get(&expression).cloned());
            let current_stable_result = current
                .result_expression
                .and_then(|expression| current_expression_stable_by_id.get(&expression).cloned());
            if stable_result != current_stable_result {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} result expression differs: direct={stable_result:?} checked={current_stable_result:?}",
                ));
            }

            let stable_metadata_matches = direct.name == current.name
                && direct.kind == current.kind
                && direct.role == current.role
                && direct.intrinsic == current.intrinsic
                && direct.external_identity == current.external_identity
                && direct.contexts == current.contexts
                && direct.contextual_operation == current.contextual_operation
                && direct.effect == current.effect
                && direct.parameters.len() == current.parameters.len()
                && direct.context_formal.is_some() == current.context_formal.is_some();
            if !stable_metadata_matches {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} stable metadata differs: direct={direct:?} checked={current:?}",
                ));
            }
            for (ordinal, (direct_parameter, current_parameter)) in direct
                .parameters
                .iter()
                ...
                .enumerate()
            {
                ...
                let metadata_matches = direct_parameter.name == current_parameter.name
                    && direct_parameter.kind == current_parameter.kind
                    && direct_parameter.ordinal == current_parameter.ordinal
                    && direct_parameter.requirement == current_parameter.requirement
                    && direct_parameter.start == current_parameter.start
                    && direct_parameter.end == current_parameter.end
                    && output_ordinal(direct_parameter.evaluation_scope, &direct.parameters)
                        == output_ordinal(current_parameter.evaluation_scope, &current.parameters)
                    && direct_declaration.map(&declaration_signature)
                        == current_declaration.map(&declaration_signature);
                if !metadata_matches {
                    mismatches.push(format!(
                        "kernel direct callable {stable_body:?} parameter {ordinal} metadata differs: direct={direct_parameter:?} checked={current_parameter:?}",
                    ));
                }
            }
            let context_metadata_matches = match (direct_context, current_context) {
                (None, None) => true,
                (Some(direct_context), Some(current_context)) => {
                    direct_context.callable == direct.decl_id
                        && current_context.callable == current.decl_id
                        && direct_context.scheme.projections
                            == boon_checked::context_scheme_projections(
                                &direct_context.scheme.flow_type.ty,
                            )
                        && current_context.scheme.projections
                            == boon_checked::context_scheme_projections(
                                &current_context.scheme.flow_type.ty,
                            )
                }
                (None, Some(_)) | (Some(_), None) => false,
            };
            if !context_metadata_matches {
                mismatches.push(format!(
                    "kernel direct callable {stable_body:?} PASSED scheme metadata differs: direct={direct_context:?} checked={current_context:?}",
                ));
            }
        }
        mismatches
    ...
    /// The old checker serialized an unconstrained empty delimiter as
    /// `Unknown`. The dense kernel retains the actual structural requirement:
    /// an open object with no fields. This allowance is restricted by the
    /// caller to the parser-issued `Delimiter` occurrence, so ordinary
    /// `Unknown` values and every nonempty structural row remain exact parity
    /// checks.
    fn legacy_contextual_delimiter_matches(kernel: &FlowType, current: &FlowType) -> bool {
        kernel.mode == current.mode
            && matches!(current.ty, Type::Unknown)
            && matches!(
                &kernel.ty,
                Type::Object(shape)
                    if shape.open && shape.fields.is_empty() && shape.field_order.is_empty()
            )
    }

    #[test]
    fn contextual_delimiter_allowance_is_exact() {
        let kernel = FlowType {
            mode: FlowMode::Continuous,
            ty: Type::object(ObjectShape::from_ordered_fields([], true)),
        };
        let legacy = FlowType {
            mode: FlowMode::Continuous,
            ty: Type::Unknown,
        };
        assert!(legacy_contextual_delimiter_matches(&kernel, &legacy));
        assert!(!legacy_contextual_delimiter_matches(
            &FlowType {
    ...
    #[test]
    fn empty_render_slot_does_not_shift_later_diagnostic_values() {
        let render_fact = |id, value| KernelStatementInput {
            id: KernelStatementId(id),
            kind: KernelStatementKind::Field {
                name: "slot".into(),
            },
            value,
            value_use: KernelStatementValueUse::RenderSlot,
            children: Box::new([]),
        };
    ...
    fn wrapped_out_calls_use_one_dense_frame_and_scoped_callback_binding() {
        ...
        let direct_repeated_outputs = oracle
            .checked_scopes
            .iter()
            .filter(|scope| scope.kind == boon_checked::CheckedScopeKind::RepeatedOutput)
            .count();
        let legacy_repeated_outputs = checked
            .scopes
            .iter()
            .filter(|scope| scope.kind == boon_checked::CheckedScopeKind::RepeatedOutput)
            .count();
        assert_eq!(direct_repeated_outputs, legacy_repeated_outputs);
        assert_eq!(
            direct_repeated_outputs, 3,
            "two callable OUT declarations and the authored bare-OUT occurrence each own one repeated-output scope",
        );
        assert_eq!(
            oracle
                .checked_declarations
    ...
    fn multiline_match_records_compile_as_structural_arm_outputs() {
        ...
        let oracle = kernel_owner_oracle(&project);
        let checked_by_stable_key = oracle
            .checked_expression_keys
            .iter()
            .zip(&checked.expressions)
            .filter_map(|(stable, expression)| {
                Some((stable.as_ref()?.clone(), expression.flow_type.clone()))
            })
            .collect::<BTreeMap<_, _>>();
        for owner in &oracle.supported {
            assert_owner_matches_current(
    ...
    #[test]
    fn user_callable_and_passed_scheme_materialize_from_one_kernel_definition() {
        let source = concat!(
            "FUNCTION contextual(value) {\n",
            "    [\n",
            "        result: value\n",
            "        mode: PASSED.settings.mode\n",
            "    ]\n",
            "}\n",
        );
        let project =
            parse_project_syntax("app/RUN.bn", [("app/RUN.bn".to_owned(), source.to_owned())])
                .expect("parse direct callable fixture");
        let report = kernel_owner_oracle(&project);
        assert!(
            report.unsupported.is_empty(),
            "direct callable fixture must compile: {:#?}",
            report.unsupported,
        );
        let parsed = parse_source("app/RUN.bn", source).expect("parse current callable fixture");
        let checked = boon_typecheck::check_program(&parsed);
        assert!(
            !checked.report.has_errors(),
            "current callable diagnostics: {:#?}",
            checked.report.diagnostics,
        );
        let (checked, _) = checked
            .program
        ...
        let current = checked
            ...
            .expect("current checker publishes contextual");
        assert_eq!(direct.name, current.name);
        ...
        assert_eq!(direct.contexts, current.contexts);
        assert_eq!(direct.contextual_operation, current.contextual_operation);
        assert_eq!(direct.effect, current.effect);
        let direct_declaration = report
            .checked_declarations
            .iter()
            .find(|declaration| declaration.id == direct.decl_id)
            .expect("direct callable declaration");
        let current_declaration = checked
            .declarations
            .iter()
            .find(|declaration| declaration.id == current.decl_id)
            .expect("current callable declaration");
        assert_eq!(
            (
                direct_declaration.kind,
                direct_declaration.name.as_str(),
                direct_declaration.span,
            ),
            (
                current_declaration.kind,
                current_declaration.name.as_str(),
                current_declaration.span,
            ),
        );
        let direct_scope = &report.checked_scopes[direct.scope_id.0 as usize];
        let current_scope = &checked.scopes[current.scope_id.0 as usize];
        assert_eq!(
            (direct_scope.kind, direct_scope.span),
            (current_scope.kind, current_scope.span),
        );
        let (direct_formals, direct_result) = alpha_normalize_callable_surface(
            &direct
                .parameters
                .iter()
                .map(|parameter| parameter.flow_type.clone())
        ...
        let (current_formals, current_result) = alpha_normalize_callable_surface(
            &current
                .parameters
                .iter()
                .map(|parameter| parameter.flow_type.clone())
        ...
        assert_eq!(direct_formals, current_formals);
        assert_eq!(direct_result, current_result);
        assert_eq!(direct.parameters.len(), current.parameters.len());
        for (direct, current) in direct.parameters.iter().zip(&current.parameters) {
            assert_eq!(direct.name, current.name);
            assert_eq!(direct.kind, current.kind);
            assert_eq!(direct.ordinal, current.ordinal);
            assert_eq!(direct.requirement, current.requirement);
            assert_eq!(direct.start, current.start);
            assert_eq!(direct.end, current.end);
        }
        let direct_body = &report.checked_statements[direct.body.unwrap().0 as usize];
        let current_body = &checked.statements[current.body.unwrap().0 as usize];
        assert_eq!(direct_body.span, current_body.span);
        let direct_result_expression =
            &report.checked_expressions[direct.result_expression.unwrap().0 as usize];
        let current_result_expression =
            &checked.expressions[current.result_expression.unwrap().0 as usize];
        assert_eq!(
            direct_result_expression.span,
            current_result_expression.span
        );

        let direct_formal = report
            .checked_context_formals
            .iter()
            .find(|formal| Some(formal.id) == direct.context_formal)
            .expect("direct PASSED scheme");
        let current_formal = checked
            .context_formals
            .iter()
            .find(|formal| Some(formal.id) == current.context_formal)
            .expect("current PASSED scheme");
        assert_eq!(
            direct_formal.scheme.projections,
            current_formal.scheme.projections
        );
        assert_eq!(
            direct_formal.scheme.projections,
            [vec!["settings".to_owned(), "mode".to_owned()]],
        );
        let direct_context = alpha_normalize_owner(
            &direct_formal.scheme.flow_type,
            std::iter::empty::<FlowType>(),
        )
        .0;
        let current_context = alpha_normalize_owner(
            &current_formal.scheme.flow_type,
            std::iter::empty::<FlowType>(),
        )
        .0;
    ...
    fn nested_owner_presentation_inherits_one_compact_enclosing_scope() {
        ...
        assert!(
            prepared
                .prepared
                .iter()
                .all(|owner| owner.payload.is_none()),
            "runtime preparation must move every source-shaped owner payload into the kernel input",
        );
        assert!(
            prepared.prepared.iter().all(|owner| {
                owner.result_expression.is_none()
                    && owner.public_child_owner_fields.is_empty()
                    && owner.generic_formal_reads.is_empty()
                    && owner.structured_delimiter_dependents.is_empty()
                    && owner.record_spread_dependents.is_empty()
                    && owner.generic_selector_dependents.is_empty()
                    && owner.detached_generic_reads.is_empty()
                    && owner.legacy_no_element_dependents.is_empty()
                    && owner.legacy_source_container_modes.is_empty()
            }),
            "runtime preparation must not retain differential-oracle source metadata",
        );
        let hold = prepared
            .definition_keys
            .iter()
...
"#;
    ...
    #[test]
    fn nested_source_container_checked_authority_matches_legacy() {
        let source = concat!(
            "store: [\n",
        ...
        let parsed = parse_source("app/RUN.bn", source).expect("parse legacy SOURCE fixture");
        ...
        let (legacy, _) = legacy.program.expect("legacy SOURCE checks").into_parts();
        let project =
            parse_project_syntax("app/RUN.bn", [("app/RUN.bn".to_owned(), source.to_owned())])
        ...
        let [kernel_source] = kernel.fields.sources.as_slice() else {
            panic!("kernel fixture must expose one SOURCE")
        };
        for (fields, source) in [(&legacy, legacy_source), (&kernel.fields, kernel_source)] {
            let declaration = fields
                .declarations
                .iter()
                .find(|declaration| declaration.id == source.declaration)
                .expect("SOURCE declaration row");
            assert_eq!(declaration.name, "events");
            assert_eq!(
                declaration.kind,
                boon_checked::CheckedDeclarationKind::Source
            );
            assert_eq!(source.path.anchor, source.declaration);
            assert_eq!(source.path.projection, ["change"]);
            assert_eq!(
                fields.expressions[source.expression.0 as usize].declaration,
    ...
    #[test]
    fn function_local_nested_source_read_authority_matches_legacy() {
        let source = concat!(
            "FUNCTION source_row() {\n",
            "    [\n",
        ...
        let parsed = parse_source("app/RUN.bn", source).expect("parse legacy function SOURCE");
        let legacy = boon_typecheck::check_program(&parsed);
        assert!(
            !legacy.report.has_errors(),
            "{:#?}",
        ...
        let (legacy, _) = legacy.program.expect("legacy function SOURCE").into_parts();
        let project =
            parse_project_syntax("app/RUN.bn", [("app/RUN.bn".to_owned(), source.to_owned())])
        ...
        for fields in [&legacy, &kernel.fields] {
            ...
            let declaration = fields
                ...
                .iter()
                .find(|declaration| declaration.id == source.declaration)
                .expect("nested SOURCE declaration");
            assert_eq!(declaration.name, "controls");
            assert_eq!(
                declaration.kind,
                boon_checked::CheckedDeclarationKind::Source
            );
            assert_eq!(source.path.anchor, source.declaration);
            assert_eq!(source.path.projection, ["select"]);
            assert_eq!(
                fields.expressions[source.expression.0 as usize].declaration,
                Some(source.declaration),
            );
                ...
                .filter(|expression| {
                    matches!(
                        ...
                        boon_checked::CheckedExpressionKind::Read {
                            ...
                            projection,
                            source: Some(read),
                        } if *target == source.declaration
                            && projection.as_slice() == ["select"]
                            && read.source == source.id
                            && read.payload_projection.is_empty()
                    )
                })
                .collect::<Vec<_>>();
            let [read] = reads.as_slice() else {
                panic!("selected field must retain one exact nested SOURCE read")
            };
            let read_declaration = fields
                .declarations
                .iter()
                .find(|declaration| Some(declaration.id) == read.declaration)
                .expect("selected read declaration");
            assert_eq!(read_declaration.name, "selected");
        }
    }

    #[test]
    fn function_local_stateful_call_authority_matches_legacy() {
        let source = concat!(
        ...
        let parsed = parse_source("app/RUN.bn", source).expect("parse legacy stateful call");
        let legacy = boon_typecheck::check_program(&parsed);
        assert!(
            !legacy.report.has_errors(),
            "{:#?}",
        ...
        let (legacy, _) = legacy.program.expect("legacy stateful call").into_parts();
        let project =
            parse_project_syntax("app/RUN.bn", [("app/RUN.bn".to_owned(), source.to_owned())])
            ...
            fields
                ...
                .map(|state| {
                    ...
                    (
                        ...
                        binding.name.clone(),
                        binding.kind,
                        state.path.projection.clone(),
                        fields.expressions[state.expression.0 as usize]
                            .declaration
                            .map(|declaration| {
                                fields
                                    .declarations
                                    .iter()
                                    .find(|candidate| candidate.id == declaration)
                                    .expect("state expression declaration")
                                    .name
                                    .clone()
                            }),
                        state.initial,
        ...
        let legacy_states = summarize(&legacy);
        let kernel_states = summarize(&kernel.fields);
        assert_eq!(kernel_states, legacy_states);
        assert_eq!(kernel_states.len(), 2);
        assert_eq!(kernel_states[0].0, CheckedStateKind::InitialLatest);
        assert_eq!(kernel_states[0].1, "completed");
        assert_eq!(kernel_states[0].5, ["state_0"]);
        assert_eq!(kernel_states[1].0, CheckedStateKind::StatefulCall);
        assert_eq!(kernel_states[1].1, "completed");
        assert!(kernel_states[1].5.is_empty());
    }
    ...
    fn novywave_kernel_timing_probe() {
        ...
        // Re-enter through the public compiler facade. This is timed outside
        // the directional kernel sample and proves that the production route
        // materializes the same stable checked rows.
        ...
        let syntax_expression_by_stable = project
            ...
            .flat_map(|owner| {
                project.owner_view(&owner).into_iter().flat_map(|view| {
                    view.expressions()
                        .zip(view.stable_expression_keys())
                        .map(|(expression, stable)| (stable, expression.id))
                        .collect::<Vec<_>>()
                })
            })
            .collect::<BTreeMap<_, _>>();
        let current_call_occurrences = fields
            ...
            .map(|call| {
                let stable = report
                    ...
                    .get(call.expression.0 as usize)
                    .and_then(Option::as_ref)
                    .expect("current checked call expression retains a stable identity");
                let syntax = syntax_expression_by_stable
                    .get(stable)
                    .copied()
                    .expect("current checked call stable identity maps to syntax");
                project
        ...
        let production_rows_are_exact = report.checked_scopes.as_ref() == fields.scopes.as_slice()
            && report.checked_declarations.as_ref() == fields.declarations.as_slice()
            && report.checked_expressions.as_ref() == fields.expressions.as_slice()
            && report.checked_statements.as_ref() == fields.statements.as_slice()
            && report.checked_callables.as_ref() == fields.callables.as_slice()
            && report.checked_context_formals.as_ref() == fields.context_formals.as_slice()
            && report.checked_calls.as_ref() == fields.calls.as_slice()
            && report.checked_call_result_paths.as_ref() == fields.call_result_paths.as_slice()
            && report.checked_pattern_bindings.as_ref() == fields.pattern_bindings.as_slice()
            && report.checked_resource_projection_requirements.as_ref()
                == fields.resource_projection_requirements.as_slice()
            && report.checked_occurrences.as_ref() == fields.occurrences.as_slice()
            && report.checked_sources.as_ref() == fields.sources.as_slice()
            && report.checked_states.as_ref() == fields.states.as_slice()
            && report.checked_lists.as_ref() == fields.lists.as_slice();
        assert!(
            production_rows_are_exact,
            "the public compiler facade must publish the exact checked rows produced by the directional kernel sample",
        );
        // The block below is retained as a detailed fail-closed diagnostic for
        // any future public-row divergence. The ordinary success path uses the
        // exact row comparison above; parser projection slots are not the ID
        // namespace of dense kernel rows.
        if !production_rows_are_exact {
            let mut checked_by_stable_key = BTreeMap::new();
            let mut checked_expression_by_stable = BTreeMap::new();
            let mut stable_by_checked_expression = BTreeMap::new();
            for (stable_key, checked_expression) in report
                .checked_expression_keys
                .iter()
                .zip(&fields.expressions)
                .filter_map(|(stable, expression)| Some((stable.as_ref()?, expression)))
            {
                checked_by_stable_key
                ...
                stable_by_checked_expression.insert(checked_expression.id, stable_key.clone());
            }
            if let Some(pattern) = std::env::var_os("BOON_KERNEL_ORACLE_TRACE_OWNER") {
                let pattern = pattern.to_string_lossy();
                for owner in report
