...
use boon_program_runtime::{
    ...
    ProgramRejection, ProgramRequestId, ProgramSessionId,
};
use boon_runtime::{
...
impl PersistentRuntime {
    ...
    pub fn set_machine_origin(
        &mut self,
        origin: boon_runtime::MachineOrigin,
    ) -> Result<(), PersistentDispatchError> {
        self.runtime
    ...
    fn prepare_distributed_turn(
        &mut self,
        immediate: bool,
        build: impl FnOnce(&mut LiveRuntime) -> boon_runtime::RuntimeResult<RuntimeTurn>,
    ) -> Result<RuntimeTurn, PersistentDispatchError> {
        self.begin_distributed_admission(immediate)?;
...
mod tests {
    ...
    use boon_program_runtime::compile_program_artifact;
    use boon_runtime::DocumentFrame;
    use boon_runtime::SourcePayload;
...
"#
    ...
    fn cells_restart_restores_one_sparse_edit_without_materializing_the_grid() {
        ...
        let formula_text = restored
            .runtime
            .project_values_current(&[boon_runtime::ValueTarget::RowField {
                row: a3,
                field: formula_field,
            }])
            .unwrap();
        assert_eq!(
            formula_text.get(&boon_runtime::ValueTarget::RowField {
                row: a3,
                field: formula_field,
            }),
        ...
        let value_field = restored
            ...
            .unwrap()
            .field_id;
        let value_target = boon_runtime::ValueTarget::RowField {
            row: a3,
            field: value_field,
        };
        let sample = restored
            .runtime
            .project_values_current(&[value_target])
            .unwrap();
    ...
    fn todomvc_restart_preserves_dynamic_row_order_and_sparse_row_authority() {
        ...
        let scenario =
            boon_runtime::parse_scenario(std::path::Path::new("../../examples/todomvc.scn"))
                .unwrap();
        let storage = SharedPersistenceDriver::default();
        let (mut runtime, _) = PersistentRuntime::from_shared_machine_plan(
            Arc::clone(&plan),
