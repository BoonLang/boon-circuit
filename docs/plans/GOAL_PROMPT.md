...
- `boon_cli run --engine plan --report ...` is quiet by default now. Legacy
  runtime and legacy-comparison replay are quarantined behind
  `diagnose-legacy-scenario`, `diagnose-plan-legacy-compare`, and
  `--diagnostic-compare-legacy`; normal refreshes should rely on report files
  and compact `jq` summaries, with `--print-report` only for intentional stdout
  JSON inspection.
