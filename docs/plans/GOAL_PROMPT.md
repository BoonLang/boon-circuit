...
- PlanExecutor command reports now keep product execution status explicit with
  `plan_executor_status` and `accepted_for_product_status`; they no longer carry
  the obsolete `comparison_status` compatibility field. Native source replay can
  use product acceptance without confusing legacy comparison state with product
  execution failure. Native replay consumers reject missing product split
  fields, so stale pre-split reports are refresh debt rather than usable product
  evidence.
