...
`Client`, `Session`, `Server`, and `SessionInfo` are reserved standard
namespaces. Application modules cannot declare those names, so a runtime API
cannot silently collide with an application helper.
...
`SessionInfo/status()` returns `Active[role, session_id]`, where `role` is
`Client`, `Session`, or `Server`. `SessionInfo/principal()` returns either
`Anonymous` or `Authenticated[id]`. These values are generated by the runtime;
they are not application state, host-injected `SOURCE` values, or durable
records.

A distributed application consists of exactly one Client graph, one
per-browser-session Session graph, and one shared Server graph. A consumer reads
a qualified exported value or calls a qualified pure function with ordinary
Boon syntax. The compiler links that expression to a typed import or call site.
In-process roles pass the runtime `Value` directly. A process or network
boundary uses the canonical bounded binary Boon wire format. HTTP, JSON, RPC,
serialization, retry envelopes, and transport handles are not exposed in Boon
source.
